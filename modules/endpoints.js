...
module.exports = app => {
  ...
  app.post("/api/product", async (request, response) => {
    ...
    if (existItem) {
      ...
      const errorMessage = {
        error: "ERROR",
        message: "Item already exists, choose another!"
      };
      ...
      const errorMessage = {
        error: "ERROR",
        message: "Experiencing problems. PLEASE BARE WITH US!"
      };
  ...
  app.post("/api/shoppingcart", async (request, response) => {
    ...
    // Operate
    const checkBoughtItem = database.shoppingcartItem(name, price, picture); // Export function from 'database.js'
  ...
  app.delete("/api/shoppingcart/:name", async (request, response) => {
    ...
    const checkBoughtItem = database.shopedItem(name); // Export function from 'database.js'
